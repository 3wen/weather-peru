# Time-varying exposure to weather shocks {#sec-lp-states}
...
$$
\begin{aligned}
    y_{c,i,{\color{wongGold}t+h}} = & {\color{wongYellow}F}\left( {\color{wongGreen}\hat{z}_{c,i,t}}\right)  \left[  \alpha_{c,i,h}^{{\color{wongLightBlue}G}} + {\color{wongOrange}\beta_{c,{\color{wongGold}h}}^{{\color{wongLightBlue}G},{\color{wongPurple}T}}} {\color{wongPurple}T_{i,{\color{wongGold}t}}} + {\color{wongOrange}\beta_{c,{\color{wongGold}h}}^{{\color{wongLightBlue}G},{\color{wongPurple}P}}} {\color{wongPurple}P_{i,{\color{wongGold}t}}}+ \delta_{c,i,h}^{{\color{wongLightBlue}G}}%
    X_{t}\right] \nonumber  \\ 
    &  +\left(  1 - {\color{wongYellow}F}\left(  {\color{wongGreen}\hat{z}_{c,i,t}}\right)  \right)  \left[  \alpha_{c,i,h}^{{\color{wongLightBlue}H}} + {\color{wongOrange}\beta_{c,{\color{wongGold}h}}^{{\color{wongLightBlue}H},{\color{wongPurple}T}}} {\color{wongPurple}T_{i,{\color{wongGold}t}}} + {\color{wongOrange}\beta_{c,{\color{wongGold}h}}^{{\color{wongLightBlue}H},{\color{wongPurple}P}}} {\color{wongPurple}P_{i,{\color{wongGold}t}}} + \delta_{c,i, h}^{{\color{wongLightBlue}H}}
    X_{t}\right]
    + \varepsilon_{c,i,t+h}
\end{aligned}
$$ {#eq-LP-seasons}
...
2.  **Growth Duration Definition**: Next, a growth duration, denoted as $T_c$, is determined. For rice and maize crops, $T_c$ is set to 7 months, while for potato and cassava crops, $T_c$ is set to 9 months.
...
3.  **Planting Area Index Calculation**: The planting area index is computed as follows:
    ...
    If $c$ is equal to "Cassava" or "Potato", then $T_c$ is set to 9; otherwise, $T_c$ is set to 7. 
...
As in [Chapter -@sec-lp], we load some functions. The three functions `get_index_longest_non_na()`{.R}, `keep_values_longest_non_na()`{.R}, and `get_data_lp()`{.R} presented in [Section -@sec-lp-functions] are defined in the R script `format_data.R` saved in the `weatherperu/R` folder.
```{r}
...
The function that performs the estimation is `estimate_linear_lp()`{.R}, as in [Section -@sec-lp-estimation]. This function is defined in the `estimations.R` R script.
```{r}
...
We can call the `estimate_linear_lp()`{.R} function to estimate the models at the different horizons.
...
Let us load the results obtained in [Section -@sec-lp-linear-export] for the linear local projection. First, let us save the current results in different objects.
