...
2. The **second part** provides the codes for replicating the local projections ([Chapters -@sec-lp-piscop], [-@sec-lp-quadratic-piscop], and [-@sec-lp-states-piscop]).
3. The **third part** gives the codes for replicating the results of the vector autoregressive model ([Chapter -@sec-var-piscop]).
4. The **fourth part** provides an analysis to examine the impacts of using quarterly data instead of monthly data in the local projections ([Chapters -@sec-merge-quarter] and [-@sec-lp-quarter]).
4. The **fifth part** provides a robustness check when using an alternative dataset for precipitation data, to study the response of agricultural production following a weather shock ([Chapter -@sec-lp-chirps]) and aggregate fluctuations ([Chapter -@sec-var-chirps]).
5. The **sixth part** considers alternative definitions of weather shocks, to account for positive vs. negative surprise shocks ([Chapter -@sec-lp-posvsnegsurprise]). It also considers removing the last year from the data ([Chapter -@sec-lp-until2014]).
