...
2.    **Retrieving land use data from [Copernicus](https://land.copernicus.eu/global/products/lc)**: this data will provide valuable information about the various land cover types present in Peru.
...
Two shapefiles were downloaded from [GEO GPS PERÙ](https://www.geogpsperu.com/2014/03/base-de-datos-peru-shapefile-shp-minam.html):

-   the [departmental boundaries](https://www.geogpsperu.com/2018/02/limite-departamental-politico-shapefile.html)
-   [a grid map of the national map](https://www.geogpsperu.com/2012/08/cuadro-de-empalme-de-la-cartografia.html).
...
```{r}
#| fig-cap: Boundaries of regions in Peru
#| label: fig-regions-map-peru
...
```{r}
#| fig-cap: Regional boundaries in Peru with the grid
#| label: fig-grid-peru-over-regions
ggplot(data = map_peru_grid) + 
  geom_sf(fill = "#00B0CA", alpha = .1, colour = "white") +
  geom_sf(data = map_peru, fill = NA)
```

## Land Use {#sec-weather-land-use}

Let us turn to the land cover data. We downloaded from [Copernicus](https://land.copernicus.eu/global/products/lc) the three land cover tiles intersecting Peru for 2015.
...
```{r}
#| fig-cap: Agricultural land use in Peru
#| label: fig-peru-land-use
p <- 
  ggplot() +
  geom_sf(
...
```{r}
#| fig-cap: Land Use in Peru as displayed on Google Earth Engine
#| label: fig-google-earth-land-use
...
The rainfall data are obtained from the [gridded daily precipitation dataset PISCOp](https://piscoprec.github.io/webPISCO/en/2018/03/04/download-using-the-senamhi-ftp/) (@aybar_2020_construction).
...
```{r}
#| code-fold: true
#| fig-cap: Monthly Mean of Daily Average Temperature 2010 in Peru
#| label: fig-grid-monthly-mean-mean-temp
p_tmp_mean <- 
  ggplot() +
  geom_sf(
...
```{r}
#| code-fold: true
#| fig-cap: Monthly Mean of Daily Minimum Temperature 2010 in Peru
#| label: fig-grid-monthly-mean-min-temp
p_tmp_min <-
  ggplot() +
  geom_sf(
...
```{r}
#| code-fold: true
#| fig-cap: Monthly Mean of Daily Maximym Temperature 2010 in Peru
#| label: fig-grid-monthly-mean-max-temp
p_tmp_max <- 
  ggplot() +
  geom_sf(
...
```{r define-weather_peru_daily_grid}
...
Following @aragon2021climate, we compute degree days, with crop-specific threshold values.
$$
...
We also define harmful degree days (@aragon2021climate):
$$
...
### Cold / Hot / Dry / Wet Surprise Days {#sec-weather-data-surprise-weather}

We follow @natoli2024temperature and define cold and hot surprise weather shocks.
...
In @natoli2024temperature, the thresholds are constructed as follows:
$$
\begin{align}
...
Then, we define function `get_surprise_w_shocks_monthly()`{.R} that computes, for a specific month $m$ in year $y$ the surprise shocks for each cell.

```{r define-get_surprise_w_shocks_monthly}
...
Since the `get_surprise_w_shocks_monthly()`{.R} function returns the surprise shocks for a single month of a year (but for all cells of the grid), we need to loop over the different months/years of the sample.
...
To convert the daily grid data into quarterly data, we aggregate the values  on a quarter basis while preserving the spatial scale of the grid cells.
...
To convert the daily grid data into annual data, we aggregate the values  on an annual basis while preserving the spatial scale of the grid cells.
*Note*: we only keep data from 1986 to 2015.
...
### Climate Normals {#sec-weather-normals}
...
@Burke_2014_EJ defines precipitation shocks as using a Gamma distribution. The historical data of monthly rainfall at each grid point are fitted to a grid-specific gamma distribution, and each year at the grid point is assigned to its corresponding percentile in that distribution. Note that @Burke_2014_EJ uses crop-year data, whereas in our analysis, we use grid-month data.


We define a function, `percentile_gamma_dist()`{.R}, to estimate the parameters of a Gamma distribution using some input values. Then, for each input value, the corresponding percentile in the estimated distribution can be returned.
```{r}
...
We aggregate the observations by grid cell and month. For each series of observations, we apply the `percentile_gamma_dist()`{.R} function.
```{r, eval=FALSE}
monthly_weather_data <- 
  monthly_weather_data |> 
  ungroup() |> 
...
In Section [@sec-weather-normals], we established climate normals. We will now calculate the deviation from those normals for each grid cell and each month.
...
The [Standardized Precipitation-Evapotranspiration Index (SPEI)](https://spei.csic.es/) is a versatile drought index that utilizes climatic data to assess the onset, duration, and severity of drought conditions compared to normal conditions. 

To calculate evapotranspiration, we need to provide the latitude parameter to the `thornthwaite()`{.R} function from the {SPIE} package. For each grid cell, we use the latitude of its barycenter as the input.
```{r, eval=FALSE}
...
We define a function, `compute_spei_cell()`{.R}, that computes the Potential evapotranspiration (PET), the Standardized Precipitation Index (SPI), and the Standardized Precipitation-Evapotranspiration Index (SPEI), at the grid level. We can specify, by giving a vector of integers to the  `scale` argument, the time scale at which the SPI and the SPEI are computed. This scale argument controls for the magnitude of the memory. In the help file of {SPEI}, one can read:
...
We can then apply the `compute_spei_cell()`{.R} function to all the grid cells
```{r, eval=FALSE}
resul_spei <- 
  map(
...
We define a function, `aggregate_region_i()`{.R}, to perform the aggregation for a specific region.
```{r, eval=FALSE}
...
Then, we apply the `aggregate_region_i()`{.R} function to each region:
```{r, eval=FALSE}
weather_regions_df <- 
  map(
...
```{r}
#| fig-cap: Average monthly temperatures in Peru (1988-2015)
#| label: fig-avg-monthly-temp-peru
#| code-fold: true
...
```{r}
#| fig-cap: Average monthly minimum temperatures in Peru (1988-2015)
#| label: fig-avg-monthly-min-temp-peru
#| code-fold: true
ggplot(data = monthly_temp_peru_monthly_avg |> 
         mutate(
           month = month.abb[month],
...
```{r}
#| fig-cap: Average monthly maximum temperatures in Peru (1988-2015)
#| label: fig-avg-monthly-max-temp-peru
#| code-fold: true
ggplot(data = monthly_temp_peru_monthly_avg |> 
         mutate(
           month = month.abb[month],
...
```{r}
#| fig-cap: Average monthly precipitation in Peru (1988-2015)
#| label: fig-avg-monthly-precip-peru
#| code-fold: true
ggplot(data = monthly_temp_peru_monthly_avg |> 
         mutate(
           month = month.abb[month],
...
```{r}
#| fig-cap: Average monthly precipitation in Peru (1988-2015)
#| label: fig-avg-monthly-precip-piscop-peru
#| code-fold: true
ggplot(data = monthly_temp_peru_monthly_avg |> 
         mutate(
           month = month.abb[month],
...
```{r}
#| fig-cap: Average monthly Degree Days in Peru (1988-2015)
#| label: fig-avg-monthly-gdd-piscop-peru
#| code-fold: true
ggplot(
...
```{r}
#| fig-cap: Average monthly Harmful Degree Days in Peru (1988-2015)
#| label: fig-avg-monthly-hdd-piscop-peru
#| code-fold: true
ggplot(
  data = monthly_temp_peru_monthly_avg |> 
    mutate(
...
```{r}
#| fig-cap: Regional mean temperatures in Peru - 2010
#| label: fig-map-mean-temp-regions
#| code-fold: true
b <- as.numeric(
  quantile(
...
```{r}
#| fig-cap: Regional minimum temperatures in Peru - 2010
#| label: fig-map-min-temp-regions
#| code-fold: true
b <- as.numeric(
  quantile(
...
```{r}
#| fig-cap: Regional maximum temperatures in Peru - 2010
#| label: fig-map-max-temp-regions
#| code-fold: true
b <- as.numeric(
  quantile(
...
```{r}
#| fig-cap: Regional precipitation in Peru - 2010
...
```{r}
#| fig-cap: Regional precipitation in Peru - 2010
#| label: fig-map-precip-piscop-regions
#| code-fold: true
b <- as.numeric(
  quantile(
...
```{r}
#| fig-cap: Regional Degree Days in Peru - 2010
#| label: fig-map-gdd-piscop-regions
#| code-fold: true
...
```{r}
#| fig-cap: Regional Harmful Degree Days in Peru - 2010
#| label: fig-map-hdd-piscop-regions
#| code-fold: true
...
We define a function, `aggregate_quarter_region_i()`{.R}, to perform the aggregation for a specific region.
```{r, eval=FALSE}
...
Then, we apply the `aggregate_quarter_region_i()`{.R} function to each region:
```{r, eval=FALSE}
...
```{r}
#| fig-cap: Average quarterly temperatures in Peru (1988-2015)
#| label: fig-avg-quarterly-temp-peru
#| code-fold: true
...
```{r}
#| fig-cap: Average quarterly minimum temperatures in Peru (1988-2015)
#| label: fig-avg-quarterly-min-temp-peru
#| code-fold: true
ggplot(data = quarterly_temp_peru_quarterly_avg |> 
         mutate(
           quarter = factor(quarter, levels = 1:4, labels = str_c("Q", 1:4))
...
```{r}
#| fig-cap: Average quarterly maximum temperatures in Peru (1988-2015)
#| label: fig-avg-quarterly-max-temp-peru
#| code-fold: true
ggplot(data = quarterly_temp_peru_quarterly_avg |> 
         mutate(
           quarter = factor(quarter, levels = 1:4, labels = str_c("Q", 1:4))
...
```{r}
#| fig-cap: Average quarterly precipitation in Peru (1988-2015)
#| label: fig-avg-quarterly-precip-peru
#| code-fold: true
ggplot(data = quarterly_temp_peru_quarterly_avg |> 
         mutate(
           quarter = factor(quarter, levels = 1:4, labels = str_c("Q", 1:4))
...
```{r}
#| fig-cap: Average quarterly precipitation in Peru (1988-2015)
#| label: fig-avg-quarterly-precip-piscop-peru
#| code-fold: true
ggplot(data = quarterly_temp_peru_quarterly_avg |> 
         mutate(
           quarter = factor(quarter, levels = 1:4, labels = str_c("Q", 1:4))
...
```{r}
#| fig-cap: Average quarterly Degree Days in Peru (1988-2015)
#| label: fig-avg-quarterly-gdd-peru
#| code-fold: true
ggplot(
...
```{r}
#| fig-cap: Average quarterly Harmful Degree Days in Peru (1988-2015)
#| label: fig-avg-quarterly-hdd-peru
#| code-fold: true
ggplot(
  data = quarterly_temp_peru_quarterly_avg |> 
    mutate(
...
```{r}
#| fig-cap: Regional mean temperatures in Peru - 2010
#| label: fig-map-mean-temp-regions-quarterly
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_mean,
...
```{r}
#| fig-cap: Regional minimum temperatures in Peru - 2010
#| label: fig-map-min-temp-regions-quarterly
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_min,
...
```{r}
#| fig-cap: Regional maximum temperatures in Peru - 2010
#| label: fig-map-max-temp-regions-quarterly
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_max,
...
```{r}
#| fig-cap: Regional precipitation in Peru - 2010
#| label: fig-map-precip-regions-quarterly
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$precip_sum,
...
```{r}
#| fig-cap: Regional precipitation in Peru - 2010
#| label: fig-map-precip-piscop-regions-quarterly
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$precip_piscop_sum,
...
```{r}
#| fig-cap: Regional Degree Days in Peru - 2010
#| label: fig-map-gdd-regions-quarterly
#| code-fold: true
map_peru_regional_weather_gdd <- 
  map_peru_regional_weather |> 
  dplyr::select(
...
```{r}
#| fig-cap: Regional Harmful Degree Days in Peru - 2010
#| label: fig-map-hdd-regions-quarterly
#| code-fold: true
map_peru_regional_weather_hdd <- 
  map_peru_regional_weather |> 
  dplyr::select(
...
We define a function, `aggregate_annual_region_i()`{.R}, to perform the aggregation for a specific region.
```{r, eval=FALSE}
...
```{r}
#| fig-cap: Average annual temperatures in Peru (1988-2015)
#| label: fig-avg-annual-temp-peru
#| code-fold: true
...
```{r}
#| fig-cap: Average annual minimum temperatures in Peru (1988-2015)
#| label: fig-avg-annual-min-temp-peru
#| code-fold: true
ggplot(data = annual_temp_peru_annual_avg) +
  geom_bar(
...
```{r}
#| fig-cap: Average annual maximum temperatures in Peru (1988-2015)
#| label: fig-avg-annual-max-temp-peru
#| code-fold: true
ggplot(data = annual_temp_peru_annual_avg) +
  geom_bar(
...
```{r}
#| fig-cap: Average annual precipitation in Peru (1988-2015)
#| label: fig-avg-annual-precip-peru
#| code-fold: true
ggplot(data = annual_temp_peru_annual_avg) +
  geom_bar(
...
```{r}
#| fig-cap: Average annual precipitation in Peru (1988-2015)
#| label: fig-avg-annual-precip-piscop-peru
#| code-fold: true
ggplot(data = annual_temp_peru_annual_avg) +
  geom_bar(
...
```{r}
#| fig-cap: Average annual Degree Days in Peru (1988-2015)
#| label: fig-avg-annual-gdd-peru
#| code-fold: true
ggplot(
...
```{r}
#| fig-cap: Average annual Harmful Degree Days in Peru (1988-2015)
#| label: fig-avg-annual-hdd-peru
#| code-fold: true
ggplot(
  data = annual_temp_peru_annual_avg |> 
    dplyr::select(
...
```{r}
#| fig-cap: Regional mean temperatures in Peru
#| label: fig-map-mean-temp-regions-annual
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_mean,
...
```{r}
#| fig-cap: Regional minimum temperatures in Peru
#| label: fig-map-min-temp-regions-annual
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_min,
...
```{r}
#| fig-cap: Regional maximum temperatures in Peru
#| label: fig-map-max-temp-regions-annual
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$temp_max,
...
```{r}
#| fig-cap: Regional precipitation in Peru
#| label: fig-map-precip-regions-annual
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$precip_sum,
...
```{r}
#| fig-cap: Regional precipitation in Peru
#| label: fig-map-precip-piscop-regions-annual
#| code-fold: true
b <- as.numeric(
  quantile(
    map_peru_regional_weather$precip_piscop_sum,
...
: Variables in the `weather_regions_df.rda` file {#tbl-desc-variables-df-weather-regions}
