# Merging: annual data {#sec-merge-annual}

:::{.callout-note}

## Objective

As in [Chapters -@sec-merge and -@sec-merge-quarter], we merge the different datasets to produce the data table used in the local projections that uses **annual data** instead of monthl or quarterly data.
...
The (annual) weather data ([Chapter -@sec-weather-data]) can be loaded:
```{r}
...
The macroeconomic data ([Chapter -@sec-data-macro]):
```{r, eval=FALSE}
...
```{r, echo=FALSE}
load("../data/output/dataset_2001_2015-annual.rda")
```
...
### Definition of the Variable of Interest {#sec-lp-pct-dev-annual}
...
-   **Step 2 (alternative version): Deviation from regional monthly average, in percent** (this step is useless in the new version of the analysis: it lead to discard too many observations)
    ...
    Once we have addressed the missing values, we proceed to the second step, which consists in computing the deviation of production from the regional average. First, we compute the average production of each crop $c$ in each region $i$:
    $$\overline{y}_{c,i} = \frac{1}{n_{T_c}} \sum_{t=1}^{T_c} y_{c,i,t}^{raw}$$
    Then, we compute the percentage deviation from this average at each date (i.e., year) $t$:
    $$y_{c,i,t} = \frac{y_{c,i,t}^{raw} - \overline{y}_{c,i}}{\overline{y}_{c,i}}$$
...
We define a function, `pct_prod_production()`{.R}, that takes the data frame of observations as input, as well as a crop name and a region ID. It returns a tibble with the following variables:

- `product_eng`: the English name of the crop
- `region_id`: the ID of the region
- `year`: year
- `y_new_normalized` (our variable of interest in [Chapter -@sec-lp-annual]): the production demeaned by the average for the crop of interest in the region of interest
