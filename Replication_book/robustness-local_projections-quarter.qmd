# Quarterly Agricultural Production (LP) {#sec-lp-quarter}
...
This chapter uses @jorda2005estimation Local Projection framework to measure how sensitive agricultural output is to exogenous changes in the weather. Instead of using monthly data as in [Chapter -@sec-lp-piscop], we use here quarterly data.
...
The data can be loaded (see [Chapter -@sec-merge-quarter])
```{r}
...
## Linear Local Projections {#sec-lp-linear-quarter}
...
### Functions {#sec-lp-functions-quarter}
...
Following the data preparation step, we proceed to define a function, `estimate_linear_lp()`{.R} that performs the estimation of models for all time horizons. This function utilizes the datasets obtained through the `get_data_lp()`{.R} function.
...
The `get_data_lp()`{.R} function is defined in the R script saved here: `/weatherperu/R/format_data-quarter.R`.
The estimation function, `estimate_linear_lp()`{.R}, is defined in the R script saved here: `/weatherperu/R/estimations-quarter.R`.
...
### Estimation {#sec-lp-estimation-quarter}
