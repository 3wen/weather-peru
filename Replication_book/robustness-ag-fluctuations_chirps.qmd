# Aggregate Fluctuations {#sec-var-chirps}

:::{.callout-note}

This Chapter reproduces the VAR analysis from [Chapter -@sec-var-piscop], using the CHIRPS precipitation data instead of the Piscop data.
...
## Synthetic measure of the weather {#sec-synthetic-measure-weather-chirps}

This section provides a detailed description of the methodology employed to create the synthetic measure of the weather.



We need to load the results from the estimations made in [Chapter -@sec-lp-chirps].
```{r}
...
## Macroeconomic indicators {#sec-var-macro-indicators-chirps}
...
-   $W_t$ represents the aggregate measure of weather-driven agricultural losses defined in @sec-synthetic-measure-weather-chirps, which quantifies the loss in agricultural value added due to weather shocks, expressed as a deviation from its trend. It is the focal variable of interest in this analysis.
...
the Exports data with token _PN01461BM_,
the GDP data with token _PN01773AM_,
the agricultural GDP with token _PN01755AM_,
and the nominal rate with token _PN07819NM_.
...
#| label: fig-var-irfs-w-chirps-repl
#| fig-width: 9
#| fig-height: 4
library(tikzDevice)
library(lemon)
...
## Comparison between PISCOp and CHIRPS data
...
```{r}
#| code-fold: true
#| fig-cap: VAR(2) system response to an unit orthogonal shock to the weather aggregate cost equation, using either PISCOp data or CHIRPS data
